...
export default {
  ...
  methods: {
    ...
    extendRoutes: function(routePermission) {
      ...
      originPath.forEach((element) => {
        this.$router.addRoute(element);
        if (element.children) {
    ...
    signin: function(callback) {
      ...
      vm.$root.userData = localUser;

      vm.extendRoutes([]);

      // if (localUser.role === Role.ROLE_SUPER) {
      //   vm.$root.menuData = AllRoutesData;
      // } else if (localUser.role === Role.ROLE_ADMIN) {
      //   vm.$root.menuData = AllRoutesData;
      // }
